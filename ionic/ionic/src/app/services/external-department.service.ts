...
export class ExternalDepartmentService {
  ...
  get_all() {
    return this.http.post("http://localhost:83/external_department","").map(res => res.json());
  }

  search_name_external_department(name: string) {
    let data = {"name":name}
    return this.http.post("http://localhost:83/external_department_search_name/", data).map(res => res.json());
  }

  search_external_department(name: string, is_active : number) {
    let data = {"name":name, "is_active" : is_active}
    return this.http.post("http://localhost:83/external_department_search/", data).map(res => res.json());
  }

  delete_external_department(department_id: number){
    let data = department_id
    return this.http.delete("http://localhost:83/external_department_delete/"+data).map(res => res.json());
  }
